...
let multiItemSlider = (function () {
	return function (selector, config, selector2) {
		...
		if (selector2) {
			let _slider2MainElem = document.querySelector(selector2);
			_sliderControls = _slider2MainElem.querySelectorAll('.miniSlider__control');
		} else {
		...
		function createItemsArr(items, count) {
			let newArr;
			if (count > 1) newArr = createSlideDupArr(items, count); // if we need more than 1 copy of slides
			newArr = fillSlideArr(items); // if we need only 1 copy of slides
			newArr[0].item.classList.add('active');
			return newArr;
		}
			...
			if (direction === 'right') {
				for (let i = 0; i < _items.length; i++) {
					if (_items[i].active) {
						...
						_items[i].item.classList.remove('active');
						if (i === _items.length - 1) {
							_items[0].active = true;
							_items[0].item.classList.add('active');
							_dots[_items[0].index].classList.add('active');
						} else {
							_items[i + 1].active = true;
							_items[i + 1].item.classList.add('active');
				...
				for (let i = 0; i < _items.length; i++) {
					if (_items[i].active) {
						...
						if (i === 0) {
							...
							_items[_items.length - 1].item.classList.add('active');
